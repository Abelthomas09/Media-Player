...
const VideoCard = ({ displayData, setDeleteVideoResponseFromVideoCard, insideCategory }) => {
  ...
  const deleteVideo = async (id) => {
    try {
      const resut = await removeVideoAPI(id)
      setDeleteVideoResponseFromVideoCard(resut)
    } catch (err) {
      console.log(err);
  ...
  const videocardDragStart = (e, dragVideoDetails) => {
    console.log("Inside videocard started with videoid: " + dragVideoDetails?.id);
  ...
  return (
    <>
      <Card draggable={true} onDragStart={e => videocardDragStart(e, displayData)} style={{ height: '250px' }}>
